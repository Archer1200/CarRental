...
const Login = () => {
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            const { data } = await axios.post(`/api/user/${state}`, { name, email, password })  // ✅ Fix 2: backticks
            if (data.success) {
                navigate('/')
                setToken(data.token)
    ...
    return (
        <>
            <div onClick={() => setShowLogin(false)} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center text-sm text-gray-600 bg-black/50'>
                ...
                {/* ✅ Fix 3: onSubmitHandler → onSubmit */}
                    ...
                    {state === "register" ? (
                        <p>Already have account? <span onClick={() => setState("login")} className="text-indigo-500 cursor-pointer">click here</span></p>
                    ) : (
